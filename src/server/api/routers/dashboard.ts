...
export const dashboardRouter = createTRPCRouter({
  getEntityCountGroupByStatus: privateProcedure
    .input(
      z.object({
    ...
    .query(async ({ ctx, input }) => {
      const getAll = async (
        ...
        if (total_count === null || total_count >= LIMIT) {
          const { total_count: newTotalCount, data: items }
            = await ctx.dnaClient
              .findAll({
                entity: input?.entity,
          ...
          const response = await getAll(
            start + LIMIT, newTotalCount ?? total_count, [...previous_items, ...(items as { id: string, status: string }[])],
          )
          return response
        }
        return {
