...
export const deviceRouter = createTRPCRouter({
  ...
  activateDevice: privateProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const accountOrganizationRecord =
        accountOrganization?.data?.[0]?.account_organizations;
      ...
      try {
        ...
        const fetch_account_secret = await ctx.redisClient.getCachedData(
          `${deviceRecord?.id}:${_account?.account_id}`,
        );
  ...
  fetchDownloadURL: privateProcedure
    ...
    .query(async ({ ctx }) => {
      ...
      const cachedUrl = await ctx.redisClient.getCachedData(
        'pfsense-package-url',
      );
  ...
  createOrganizationAccount: privateProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const account_response = await ctx.dnaClient
        .findAll({
          ...
          query: {
            ...
            pluck_object: {},
            advance_filters: createAdvancedFilter({
              id: find_res?.data[0]?.account_organizations?.account_id,
            }),
            order: {
              limit: 1,
              by_field: 'created_date',
      ...
      if (!account_response?.data?.length) {
        ...
        await ctx.redisClient.cacheData(`${id}:${account_id}`, {
          account_secret,
