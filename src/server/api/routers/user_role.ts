...
/** account_organizations rows whose role_id matches (or deliberately does not match) a role. */
...
const pluckContactIds = (rows: Record<string, any>[]) => [
  ...new Set(
    rows
      .map(
        (row) => row?.contact_id ?? row?.account_organizations?.[0]?.contact_id,
      )
...
export const userRolesRouter = createTRPCRouter({
  ...
  assignUsers: privateProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const account_organization_ids = (
        (response?.data ?? []) as Record<string, any>[]
      )
        .map((row) => row?.id ?? row?.account_organizations?.[0]?.id)
        .filter(Boolean) as string[];
