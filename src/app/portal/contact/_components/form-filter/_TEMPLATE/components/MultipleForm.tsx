...
const MultipleForm: React.FC<IMultipleFormProps> = (props) => {
  ...
  const [editMode, setEditMode] = useState(() => {
    ...
    const mapped = records?.map((_: any, index: any) => {
      return {
        index,
        in_edit_mode: true,
      };
  ...
  const handleOnClickSubmit = async (
    ...
    // ? Use this after mutation success
    // ? This is to toggle the selected view to form mode
    // form?.setValue(`${GLOBAL_PARENT_VARIABLE_KEY}.${index}.in_edit_mode`, false);
  ...
  const handleSelectedGridRecords = (
    ...
    // form.setValue(`${GLOBAL_PARENT_VARIABLE_KEY}.${index}.first_name`, "Test");
    // form.setValue(`${GLOBAL_PARENT_VARIABLE_KEY}.${index}.in_edit_mode`, false);
    handleToggleEditMode(index, false);
      ...
      {form
        ...
        [GLOBAL_PARENT_VARIABLE_KEY]?.map(
          (field: Record<string, any>, index: number, array: Array<any>) => {
            ...
            const isEditMode = editMode?.find((item) => item?.index === index);
            return (
