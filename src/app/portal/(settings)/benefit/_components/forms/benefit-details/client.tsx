...
export default function BenefitDetails({ params, defaultValues }: IFormProps) {
  const toast = useToast();
  const updateBenefit = api.benefit.update.useMutation();

  const handleSave = async ({
    data,
  }: IHandleSubmit<z.infer<typeof FormSchema>>) => {
    try {
      const res = await updateBenefit.mutateAsync({
        id: params.id,
        ...data,
      });
