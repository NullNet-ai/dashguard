...
const LoginOrganization = async ({
  ...
  const fetchedOrganizations =
    await api.organizationContact.fetchOrganizationsWithPrimaryOrg({
      contact_id,
    });
  const { data } = fetchedOrganizations ?? {};
  const organization_list = data?.map((org: Record<string, any>) => {
