...
/**
 * File upload Docs: {@link: http://localhost:3000/docs/file-upload}
 */
...
>(({ className, index, children, file, onFileUploaded }, ref) => {
  ...
  const onProgress = (progressEvent: AxiosProgressEvent) => {
    const percentCompleted = Math.round(
      // @ts-expect-error - TS doesn't know about loaded and total
  ...
  const onUploadClick = () => {
    ...
    uploader.current
      ...
      .then((response) => {
        ...
        onFileUploaded(response.data.data[0].id);
      })
      .catch(() => {
