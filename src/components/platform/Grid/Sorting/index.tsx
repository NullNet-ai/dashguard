    ...
    >
      ...
      {state?.sorting?.map((item: ColumnSort, index) => {
        ...
        return (
          <Badge
            className={cn(
              `item-ref m-1 flex items-center gap-1 whitespace-nowrap`, { 'opacity-0': isHidden },
            )}
            key={item.id}
            ref={(el) => {
              if (el) {
      ...
      <Button
        className={cn(
          `h-[30px] text-default/60 underline hover:no-underline`, `${data?.length && data.some(item => item.hidden) ? 'absolute mt-[2px]' : ''}`,
        )}
