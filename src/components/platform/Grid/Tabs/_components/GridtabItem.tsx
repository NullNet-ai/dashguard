...
const GridTabItem = forwardRef<HTMLDivElement, InnerTabitemProps>(
  (
    { tab, pathname, newItems, className, isHidden, onClickItem, actions },
    ref,
  ) => {
    ...
    const handleClickLink = async (tabid?: string) => {
      ...
      const newItem = [...newItems].map((item) => {
        return {
          ...
          is_current: item.id === tab.id,
        };
      });

      const sorted = reorderGridTabActive(
        newItem,
        tabid ?? '',
        application ?? '',
      );
      ...
      const cachedItems = JSON.parse(
        localStorage.getItem('cachedPortalItems') || '{}',
      );
    ...
    return (
      <div
        ...
        key={tabNameRole}
        className={cn(
          `group relative flex h-[36px] items-center whitespace-nowrap rounded-md bg-tertiary md:h-[32px]`,
          `${isGrid ? 'pl-0' : 'pl-[8px]'} `,
          className,
        )}
      >
        {!lowerCase(tab.name)?.includes('all') &&
        !lowerCase(tab.name)?.includes(lowerCase(entityName || '')) ? (
        ...
        <Link
          ...
          className={cn(
            tab.current ? 'text-primary' : 'text-default/70',
            'whitespace-nowrap text-sm font-medium',
            'flex items-center space-x-2',
            'hover:border-t-primary hover:text-primary',
            `${isGrid ? 'px-[8px] pr-0' : 'pr-0'}`,
