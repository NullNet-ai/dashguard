...
export default function SearchDialogTimeline() {
  ...
  const { handleSearchQuery } = actions ?? {};
    ...
    const _advanceFilterItems = advanceFilterItems
        .map((item) => {
  ...
  const debouncedSearchInput = useDebounce(query, 500);
    ...
    const searchFields = constructSearchableFields({
  ...
  const data = handleSearchQuery!(
    {
      ...
      advance_filters: _advanceFilterItems as IAdvanceFilters[],
