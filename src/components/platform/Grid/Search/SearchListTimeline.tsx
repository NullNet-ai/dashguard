...
const SearchListTimeline = ({ parentType }: any) => {
  ...
  useEffect(() => {
    ...
    const calc = (items?: any[]) => {
      ...
      const styleUpdates: Array<{
        element: HTMLElement;
        styles: { position: string; left: string; pointerEvents: string };
      }> = [];
  ...
  return (
    <div
      className={cn(
        `container-ref relative flex flex-col items-center gap-2 overflow-hidden md:flex-row`,
      )}
      ref={conref}
    >
      <div className="flex flex-1 flex-row items-center justify-between">
        {defaultSearchItems.length ? (
          <div className="flex max-w-[387px] flex-1 flex-row items-center gap-x-[5px]">
            <span
              className={cn(
            ...
            {defaultSearchItems.length ? (
              <div className="relative flex flex-nowrap items-center gap-x-[5px]">
                {defaultSearchItems?.map((item: any, index: number) => {
                  ...
                  if (item?.is_custom_value) {
                    searchText = shortentext(
                      item.type === 'criteria'
                        ? `${formatAndCapitalize(item?.key ?? '') || item?.label} is "${item?.custom_value ?? item?.display_value}"`
                        : item?.operator,
                      path2 === 'record' || path2 === 'wizard' ? 10 : 25,
                    );
                  }
                  ...
                  <div className="max-w-[63px]">
                    ...
                    >
                      ...
                      <DropdownMenuContent align="end" side="bottom">
                        <div className="flex flex-col gap-1 gap-y-2 px-2 py-2">
                          {data?.map((item, index) => {
                            if (
                              !item.hidden ||
                              item.type === 'operator' ||
                                        ...
                                        !item.default && (
                                          <Button
                                            ...
                                            onClick={() => {
                                              ...
                                              actions?.handleRemoveSearchItem(
                                                item,
                                              );
                                            }}
                                          >
...
export default SearchListTimeline;
