...
export default function SelectedActions(
  ...
  const { enableLockFormCopy = true, enableLockFormEllipsis = true, enableLockFormView = true} = features ?? {}

  if(!enableLockFormEllipsis || (!enableLockFormCopy && !enableLockFormView)) return null;
  return (
    <DropdownMenu>
      <DropdownMenuTrigger>
      ...
      <DropdownMenuContent align="end">
       ...
       {enableLockFormCopy && <DropdownMenuItem
          onClick={async() => {
        ...
        {
          !!customFormFilterLockFormActions?.length && customFormFilterLockFormActions.map((action, index) => (
            <DropdownMenuItem
              key={index}
