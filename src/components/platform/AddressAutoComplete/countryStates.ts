import countryWithCodes from "./countryCodes";
...
const data: CountryData[] = [
  ...
  code:
    e.code ||
    (
      find(countryWithCodes, { name: e.country }) || { code: "" }
    ).code.toLowerCase(),
}));
...
export const getCountries = (filterFn = (list: any) => list) =>
  filterFn(data).map(({ country }: { country: string }) => country);

export const getStates = (
  country: string,
  stateTarget: keyof CountryData = "states",
) => {
  if (!country) {
    return [];
  }
  const found = data.find((e) => e.country === country);
  return found ? found[stateTarget] : [];
};
export const getAbvStates = (state: keyof typeof abvStates) =>
  state ? abvStates[state.toUpperCase() as keyof typeof abvStates] : "";
export const getAbvCountries = (country: string) =>
  country ? abvCountry[country.toUpperCase() as keyof typeof abvCountry] : "";

export const stateList = getStates;
