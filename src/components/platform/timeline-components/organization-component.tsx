...
const getKeyLabel = (key: string): string => {
  ...
  //capital each word
  return override ? override.label : formatTextUnderScoreToSpace(key)?.split(' ').map(word => capitalize(word)).join(' ')
};
...
const OrganizationComponent = ({
  ...
  const org_id = item?.new_value?.contact_organization_id ?? undefined

  const {data: _org_data } = api.record.getById.useQuery(
    { id: org_id, main_entity: 'organizations', pluck_fields: ['code', 'id', 'name']},
    { enabled: !!org_id }
  );

  const {name: org_name = ''} = _org_data?.data ?? {}
  ...
  const renderChildLabel = (key: string) => {
    ...
    return (
      <div data-meta={org_name} className={`mb-2 flex items-center custom-organization-component gap-2 ref-id-${item.id}`}>
        <span className="text-sm font-medium capitalize text-gray-900">
    ...
    <>
      ...
      {item.action === 'INSERT' &&
        (() => {
          ...
          if(org_name) {
            // Insert the organization name as the first entry
            displayedEntries.push(['organization name', org_name]);
          }
...
OrganizationComponent.displayName = 'OrganizationComponent';
