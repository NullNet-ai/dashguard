...
const GridFilterComponent = ({
  ...
  const { referer } = item?.metadata ?? {};
  const parentEntity = referer?.entity || '';
  ...
  const renderItem = (key: string, value: any, isPrevValue?: boolean) => {
    if (key === 'columns') {
              ...
              <div key={index} className="flex items-center gap-1">
                <div
                  className={cn('text-success', {
                    'text-danger line-through': isPrevValue,
                  })}
                >
                  {capitalize(
                    formatTextUnderScoreToSpace(val.sort_key || val.label),
                  )}
                  :
                </div>
                <div
                  className={cn({ 'text-danger line-through': isPrevValue })}
                >
      ...
      return (
        <div
          className={`font-medium ${isPrevValue ? 'text-danger line-through' : 'text-success'}`}
        >
          {isPrevValue ? 'null' : item.responsible_account_full_name}
        </div>
      );
    } 
      ...
      <div className="flex gap-8 text-sm">
        {isUpdate && (
          <div className="flex-1 rounded-sm bg-slate-100 p-2">
            <label className="font-semibold text-gray-600">Updated Date</label>
            <p className="flex gap-1 font-medium">
              {renderItem('record_created_date', item?.record_created_date, true)}{' '}
              {renderItem('record_created_time', item?.record_created_time, true)}
            </p>
          </div>
        )}
        <div className="flex-1 rounded-sm bg-slate-100 p-2">
  ...
  return (
    <>
      <div className={`mb-2 flex items-center gap-2 item-component-grid-filter-custom ref-id-${item.id} action-${toPascalCase(item.action.toLowerCase())}`}>
        <span className="text-sm font-medium capitalize text-gray-900">
          {formatTextUnderScoreToSpace(
            `${parentEntity} ${singular(item.table)}`,
          )}
        </span>
