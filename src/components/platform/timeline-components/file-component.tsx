...
const FileComponent = ({
  ...
  const getImageUrl = async () => {
    ...
    const base_url = `${pro_url}${item?.new_value?.download_path}`
    
    console.info("🚀 ~ getImageUrl ~ pro_url:", pro_url, item?.new_value?.download_path)
    try {
      if(!base_url) {
        return '';
      }
  ...
  const renderValue = (value: any, key: string, isOldValue?: boolean) => {
    ...
    if(typeof value === 'string') {
      ...
      if(key==='originalname') {
        return <button className='cursor-pointer text-sm text-blue-500 underline'
          onClick={ async () => {
  ...
  const renderChildLabel = (key: string) => {
    ...
    return (
      <div className={`mb-2 flex items-center file-default-component gap-2 ref-id-${item.id}`}>
        <span className="text-sm font-medium capitalize text-gray-900">
  ...
  return (
    <>
      ...
      {/* {item.new_value && (
        ...
        >
          <DialogContent className="w-1/2 bg-white md:w-2/3 max-w-[600px]">
            Dialog content
          </DialogContent>
        </Dialog>
    </>
