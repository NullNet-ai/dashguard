...
const InnerTabItems = ({ tabs, pathname }: InnerTabItemsProps) => {   
    ...
    return (
      ...
      aria-label="Tabs"
      className={cn(
        "scrollbar-hide flex justify-between border-b gap-x-2",
      )}
    >
      <div className="flex items-center">
        {newItems.map((tab) => (
          <div
            key={tab.name}
            className="group relative flex items-center px-2 pr-1 py-2"
          >
            <a
              data-test-id={
                "innerTab" + capitalize(tab.name).split(" ").join("")
              }
              href={tab.href}
            ...
            <TabMenu current={tab.href.match(pathname) ? true : false} href={tab.href} tabs={newItems} name={tab.name} />
          </div>
        ))}
      </div>
