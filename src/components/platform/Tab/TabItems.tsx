...
const TabItems = ({ items = [] }: TabItemsProps) => {
  ...
  useEffect(() => {
    const calc = (params?: any[]) => {
      ...
      const result = calculateMainTabItems(
        allItems,
        containerWidth,
        'dashboard',
      );
  ...
  const closeAllTabs = () => {
    const newTablist = tablists.filter(
      (item: any) => item.id === 'dashboard' || item.name === 'dashboard',
    );
    setTablists(newTablist);
    updatecachedItems(newTablist);
  ...
  return (
    <nav
      aria-label="Tabs"
      className={cn(
        'scrollbar-hide fixed top-[57px] z-[49] flex w-full justify-between gap-x-2 border-b border-slate-100 bg-white pl-0 pt-[4px] md:static md:min-h-[2.0rem] md:border-b-0 md:bg-none md:pt-[0] lg:pl-0',
      )}
    >
      <>
        <div
