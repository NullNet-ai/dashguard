      ...
      <SidebarContext.Provider value={contextValue}>
        <TooltipProvider delayDuration={0}>
          <div
            ...
            className={cn(
              "group/sidebar-wrapper flex min-h-svh w-full overflow-hidden text-sidebar-foreground has-[[data-variant=inset]]:bg-sidebar",
              className,
            )}
            ref={ref}
            {...props}
...
>(({ className, onClick, iconClassName, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant="ghost"
      className={cn(
        "h-7 w-7 hover:bg-transparent active:translate-y-0",
        className,
      )}
      onClick={(event) => {
        onClick?.(event);
...
const SidebarInset = React.forwardRef<HTMLDivElement, SidebarInsetProps>(
  ({ className, application_name, ...props }, ref) => {
    const mt =
      application_name === "record" ? "lg:mt-[4.4rem]" : "lg:mt-[4.9rem]";
