...
import { getStringFormat } from './util/get-format';
...
const DefaultComponent = ({
 ...
 const handleClick = async () => {
     ...
     if(identifier === undefined || entity === undefined) {
     ...
     if (record?.status) {
       ...
       let url = `/portal/${entity}/${application}/${identifier}/1`;
       if (application === 'record') {
         url = `/portal/${entity}/${application}/${identifier}/timeline`;
       }
  ...
  const renderChildLabel = (key: string) => {
        ...
        const handleParentClick = async () => {
          ...
          if (record?.status) {
            ...
            const url = application === 'record' 
              ? `/portal/${config.entity}/record/${parentObject.code}/timeline`
    ...
    return (
      <div className={`mb-2 flex items-center default-item gap-2 ref-id-${item.id}`}>
        <span className="text-sm font-medium capitalize text-gray-900">
        ...
        {parentTable && item.parent_details?.[singular(parentTable)]?.code && (
          <button
            type="button"
            onClick={handleClick}
            className="cursor-pointer text-sm text-blue-500 underline"
          >
            ({item.parent_details?.[singular(parentTable)]?.code})
          </button>
        )}
      </div>
    );
              ...
              {displayedEntries.map(([key, value]) => (
                  <div key={key} className="flex gap-8 text-sm">
                    <div className="flex-1 rounded-sm bg-slate-100 p-2">
                      ...
                      <div
                        className={`font-medium ${'text-danger line-through'}`}
                      >
