...
export const abbrValue = (value: number, decimalSeparator = '.', _decimalPlaces = 10): string => {
  if (value > 999) {
    ...
    const suffix = ' kMGTPE'[valueLength / 3] || '';
    const abbrValue = Math.round((value * d) / p(10, valueLength)) / d + suffix;
...
export const parseAbbrValue = (value: string, decimalSeparator = '.'): number | undefined => {
  ...
  if (match) {
    ...
    const multiplier = abbr ? abbrMap[abbr.toLowerCase()] : undefined;

    return digits && multiplier ? Number(digits.replace(decimalSeparator, '.')) * multiplier : undefined;
  }
