  ...
  const handleParse = React.useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (parsedDateTime) {
        ...
        // Set value to invalid date if the input is not a valid date
        onValueChange("Invalid Date");
    ...
    (e: React.KeyboardEvent<HTMLInputElement>) => {
      if (e.key === "Enter") {
        ...
        if (parsedDateTime) {
          ...
          onValueChange(parsedDateTime);
          setInputValue(formatDateTime(parsedDateTime));
          onTimeChange && onTimeChange(`${hour}:${parsedDateTime.getMinutes()} ${PM_AM}`);
        }
      }
    },
...
const DateTimeLocalInput = ({
  ...
  disabled,
  ...props
}: DateTimeLocalInputProps &{ minDate?: Date;
  maxDate?: Date;
  disablePastDates?: boolean;
  disableFutureDates?: boolean;
  includeTime?: boolean;
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button
          ...
          className={cn(
            "size-9 flex items-center justify-center font-normal  disabled:opacity-100",
